...
def schedule_interview(candidate: str, start_time: str, job_title: str = "Candidate", duration_minutes: int = 30, user=None) -> str:
    ...
    start_time: ISO 8601 datetime string (e.g., '2026-03-02T15:00:00').
    ...
    try:
        ...
        try:
            ...
            try:
                google_service = service.get_service()
                tz = google_service.settings().get(setting="timezone").execute().get("value", "Asia/Kolkata")
            except Exception:
                tz = "Asia/Kolkata"
            ...
            print(f"Calendar Invite failed: {cal_err}")
