...
graph TB
    ...
    subgraph "Harvey Local Architecture"
        ...
        TOOLS[Harvey AI Tools<br/>(schedule_interview)]
    end
...
- **`base_url` & `auth_token`**: Connection credentials (token is encrypted at rest).
- **Legacy Endpoints**: Standard routes for basic entities (`departments_endpoint`, `employees_endpoint`, `jobs_endpoint`, etc.).
- **Security Lock**: A 16-character `edit_token` is required to unlock this configuration in the Admin UI, valid for strictly 24 hours.
...
- **`sample_json`**: Used by the backend to validate incoming payloads against Harvey's strict schema validators before the mapping can be saved.
...
Acts as a dynamic factory. Given an `hrms_type` string (from the `HRMSSystemConfig`), it returns the correct initialized adapter class. This allows seamless plug-and-play addition of new integrations without modifying the core sync logic.
...
The actual data movement happens in a Celery task named `sync_organization_data(org_id)`.
...
sequenceDiagram
    ...
    alt Sync Running or Cooldown < 15m
        ...
        loop For each page in get_interviews()
            ...
            Celery->>DB: Upsert Interview resolving ForeingKeys
        end
...
1. **State Safety:** Before doing any work, the task verifies no other sync is running for this organization using Redis (`SyncStatusTracker`).
...
3. **Relationship Mapping:** External mocks often use UUIDs (e.g., `EMP001`), while Harvey relies on strict database joins (usually by `email`). During the Employee and Candidate sync phases, the script builds an in-memory dictionary mapping external IDs to actual emails, which are subsequently used to resolve Foreign Keys when syncing Interviews.
4. **Dynamic Dispatching:** After legacy syncing, it scans all `HRMSEndpointMapping` rows. It issues a `GET` request to each URL, parses the target model, and sends the JSON dict to dedicated internal upserters (e.g., `_upsert_leave_request`).

---

## 5. Interaction with AI Tools

Because Harvey syncs data into its *local* database, AI execution is radically accelerated.

When the LLM decides to use the `schedule_interview` tool:
1. The tool queries parameters directly against the local, replicated database (ensuring instantly available data).
2. It pushes the new `Interview` record directly into the local PostgreSQL database.
3. (In the future state/bidirectional sync pattern) This new record is queued in a Dead Letter Queue or Outbox pattern, and a background worker subsequently PUSHES the state change backward into the HRMS asynchronously. 

This model essentially decouples the LLM's speed from external API latencies.
